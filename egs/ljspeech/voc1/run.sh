...
#conf=conf/parallel_wavegan.v1.causal.yaml
#conf=conf/parallel_wavegan.v3.yaml
#conf=conf/parallel_wavegan.small.yaml
...
#data_dir=data2
#TODO
dumpdir=/workspace/ssd2/pwg/dump           # directory to dump features
#dumpdir=/workspace/ssd2/mbmel/dump           # directory to dump features

# training related setting
# TODO
#tag=""     # tag for directory to save model
tag="mbmel_test"     # tag for directory to save model
tag="debug_mbmel"     # tag for directory to save model
...
#checkpoint='./exp/ljspeech_multi_band_melgan.v2/checkpoint-1000000steps.pkl'
...
if [ "${stage}" -le 0 ] && [ "${stop_stage}" -ge 0 ]; then
    ...
    local/data_prep.sh \
        ...
        --dev_set "${dev_set}" \
        --eval_set "${eval_set}" \
        "${download_dir}/LJSpeech-1.1" ${data_dir}
fi
...
if [ "${stage}" -le 2 ] && [ "${stop_stage}" -ge 2 ]; then
    ...
    # utils/run.pl --gpu 1 exp/train_nodev_ljspeech_test/train.log parallel-wavegan-train --config conf/parallel_wavegan.v1.yaml --train-dumpdir /workspace/ssd2/pwg/dump/train_nodev/norm --dev-dumpdir /workspace/ssd2/pwg/dump/dev/norm --outdir exp/train_nodev_ljspeech_test --resume  --verbose 1
    echo ${cuda_cmd} --gpu "${n_gpus}" "${expdir}/train.log" \
        ${train} \
            --config "${conf}" \
