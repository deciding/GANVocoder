...
import parallel_wavegan.trainers
...
def main():
    ...
    if args.train_dumpdir is not None:
        train_dataset = AudioMelDataset(
            ...
            return_spk_id=config.get("return_spk_id", False),  # keep compatibility
        )
    else:
    ...
    trainer_class = getattr(
        parallel_wavegan.trainers,
        # keep compatibility
        config.get("trainer_type", "Trainer"),
    )
    collater_class = getattr(
        parallel_wavegan.trainers,
        # keep compatibility
        config.get("collater_type", "Collater"),
    )
